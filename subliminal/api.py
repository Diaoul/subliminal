...
def list_subtitles(paths, languages=[], services=None, force=True, multi=False, cache_dir=None, max_depth=3):
    ...
    languages = set(map(guessit.Language, languages) or ALL_LANGUAGES)
...
def download_subtitles(paths, languages=[], services=None, force=True, multi=False, cache_dir=None, max_depth=3, order=None):
    ...
    """
    services = services or SERVICES
    languages = map(guessit.Language, languages) or list(ALL_LANGUAGES)
    if isinstance(paths, basestring):
        paths = [paths]
